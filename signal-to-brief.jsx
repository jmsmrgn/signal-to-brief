...
async function callClaude(systemPrompt, userMsg, useSearch = false) {
  const body = {
  ...
  const scan = async () => {
    ...
    try {
      ...
      const rawBriefs = await callClaude(
        buildExtractionPrompt(topic, subs),
        `Research ${topic} in: ${subs.join(", ")}`,
        true
      );
      const briefArr = JSON.parse(rawBriefs);
      setBriefs(Array.isArray(briefArr) ? briefArr : []);
      setView("results");
    } catch (e) {
      setErr(e.message || "Signal extraction failed. Check API connection.");
      setView("error");
    }
  };
          ...
          <div className="input-view">
            ...
            <div className="input-wrap">
              <input
                ...
                onKeyDown={(e) => e.key === "Enter" && topic.trim() && scan()}
